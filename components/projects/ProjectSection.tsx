...
export const ProjectsSection = () => {
  const projects = [
    ...
    },
    {
      ...
      description: 'Data visualization platform with machine learning insights, built with React and Python backend.',
      ...
      <div className="container mx-auto px-6">
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-gray-400 max-w-2xl mx-auto">
            A showcase of my recent work in web development, mobile apps, and software solutions
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {projects.map((project, index) => (
            <Card key={index} className="bg-slate-900/50 border-slate-700 hover:border-slate-600 transition-all duration-300 group">
              <div className="relative overflow-hidden rounded-t-lg">
                ...
                />
                <div className={`absolute top-4 right-4 px-2 py-1 rounded-full text-xs font-semibold text-white ${getStatusColor(project.status)}`}>
                  {project.status}
                </div>
              </div>
